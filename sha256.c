...
/*
 * Copied from efitools rev. 190cf3d with minor modifications for style and
 * portability.
 */
...
void sha256_process(struct sha256_context *ctx, uint8_t data[64])
{
...
void sha256_finish(struct sha256_context *ctx, uint8_t digest[SHA256_DIGEST_SIZE])
{
	uint32_t last, padn;
	uint32_t high, low;
	uint8_t msglen[8];
...
{
	struct PE_COFF_LOADER_IMAGE_CONTEXT context;
	struct sha256_context ctx;
	void *hashbase;
	unsigned int hashsize;
	struct EFI_IMAGE_SECTION_HEADER *section;
	struct EFI_IMAGE_SECTION_HEADER **sections;
	...
	sections = malloc(context.NumberOfSections * sizeof(*sections));
	if (!sections)
	...
	hashbase = buffer;
	hashsize = (uint8_t *)checksum_ptr - (uint8_t *)buffer;

	sha256_update(&ctx, hashbase, hashsize);

	/* hash post-checksum to start of certificate table */
	hashbase = (uint8_t *)checksum_ptr + checksum_size;
	hashsize = (uint8_t *)context.SecDir - (uint8_t *)hashbase;
	...
	hashsize = context.SizeOfHeaders -
		(int) ((uint8_t *)hashbase - (uint8_t *)buffer);

	sha256_update(&ctx, hashbase, hashsize);
	sum_of_bytes = context.SizeOfHeaders;
	section = (struct EFI_IMAGE_SECTION_HEADER *) ((char *) context.PEHdr
			+ sizeof (UINT32)
	...
	if (DataSize > sum_of_bytes + context.SecDir->Size) {
		...
		hashbase = (uint8_t *)buffer + sum_of_bytes;
		...
		printf("Invalid Data Size %d bytes too small\n",
